...
export class DocumentationGenerator {
  ...
  private project(scan: ScanResult, agentsSummary: string): string {
    return [
      metadata(scan),
  ...
  private architecture(scan: ScanResult): string {
    return [
      metadata(scan),
      ...
      '## Routing Modules',
      symbolTable(
        uniqueBy(scan.routes.map((route) => ({ name: route.filePath, filePath: route.filePath, exported: false }))),
        ['Routing File', 'Path']
      ),
    ].join('\n');
  }

  private commonServices(scan: ScanResult): string {
  ...
  private components(scan: ScanResult): string {
    return [
      metadata(scan),
  ...
  private apiGuidelines(scan: ScanResult): string {
    return [
      metadata(scan),
  ...
  private store(scan: ScanResult): string {
    return [
      metadata(scan),
      '## NgRx Store Domains',
  ...
  private models(scan: ScanResult): string {
    return [
      metadata(scan),
  ...
  private routes(scan: ScanResult): string {
    return [
      metadata(scan),
      '## Routes',
  ...
  private commonUtils(scan: ScanResult): string {
    return [
      metadata(scan),
      '## Shared Utilities',
  ...
  private errorHandling(scan: ScanResult): string {
    const likelyNotificationServices = scan.services.filter((service) =>
      /notification|error|alert|toast/i.test(service.name)
    );
  ...
  private checklist(scan: ScanResult): string {
    return [
      metadata(scan),
      '## Before Implementing',
      checklist([
...
function metadata(scan: ScanResult): string {
  return [
...
function componentTable(components: readonly ComponentSymbol[]): string {
  return table(
    ['Component', 'Selector', 'Path', 'Inputs', 'Outputs'],
    limitRows(components).map((component) => [
      component.name,
      component.selector ?? '-',
      component.filePath,
...
function serviceTable(services: readonly ServiceSymbol[]): string {
  return table(
    ['Service', 'Path', 'Methods', 'Dependencies'],
    limitRows(services).map((service) => [
      service.name,
      service.filePath,
...
function apiTable(usages: readonly ApiUsage[]): string {
  return table(
    ['Owner', 'Method', 'Target', 'Common Service', 'Path'],
    limitRows(usages).map((usage) => [
      usage.owner,
...
function storeTable(store: readonly StoreSlice[]): string {
  return table(
    ['Domain', 'Directory', 'Actions', 'Reducers', 'Effects'],
    limitRows(store).map((slice) => [
      slice.name,
      slice.directory,
...
function modelTable(models: readonly ModelSymbol[]): string {
  return table(
    ['Name', 'Kind', 'Path', 'Exported'],
    limitRows(models).map((model) => [model.name, model.kind, model.filePath, model.exported ? 'yes' : 'no'])
  );
}

function routeTable(routes: readonly RouteSymbol[]): string {
  return table(
    ['Route', 'Component', 'Load Children', 'Redirect', 'Guards', 'Path'],
    limitRows(routes).map((route) => [
      route.path,
      route.component ?? '-',
      route.loadChildren ?? '-',
      route.redirectTo ?? '-',
...
function utilityTable(utilities: readonly UtilitySymbol[]): string {
  return table(
    ['Name', 'Kind', 'Path'],
    limitRows(utilities).map((utility) => [utility.name, utility.kind, utility.filePath])
  );
}

function symbolTable(symbols: readonly AngularSymbol[], headers: readonly string[]): string {
...
function limitRows<T>(items: readonly T[], limit = 250): readonly T[] {
  return items.slice(0, limit);
}

function uniqueBy(items: readonly AngularSymbol[]): readonly AngularSymbol[] {
  const byPath = new Map<string, AngularSymbol>();
