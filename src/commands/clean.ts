...
export async function cleanCommand(options: CommandOptions): Promise<void> {
  ...
  const targets = [...new Set([...defaultTargets, ...state.installedFiles.map((record) => record.path)])].filter(
...
async function removeEmptyParents(root: string, startDir: string): Promise<void> {
