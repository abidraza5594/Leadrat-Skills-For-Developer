...
import type { CommandOptions, DoctorFinding } from '../core/types.js';
...
export async function doctorCommand(options: CommandOptions): Promise<void> {
  ...
  for (const target of TEMPLATE_TARGETS) {
    await checkRequiredFile(
      path.join(context.repoRoot, target.target),
      findings,
      'CONFIG_MISSING',
      `Missing AI assistant config: ${target.target}`,
      target.target
    );
  }
  ...
  for (const doc of DOCUMENTATION_FILES) {
    await checkRequiredFile(
      path.join(context.repoRoot, doc.fileName),
      findings,
      'DOC_MISSING',
      `Missing AI knowledge document: ${doc.fileName}`,
      doc.fileName
    );
  }
...
async function checkAssistantStructure(repoRoot: string, findings: DoctorFinding[]): Promise<void> {
...
async function checkOutdatedDocs(repoRoot: string, findings: DoctorFinding[]): Promise<void> {
...
function renderFindings(findings: readonly DoctorFinding[]): void {
  if (!findings.length) {
