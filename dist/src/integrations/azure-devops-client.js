...
export class AzureDevOpsClient {
    ...
    async getWorkItem(referenceText, options = {}) {
        ...
        if (!response.ok) {
            throw new AssistantError(`Failed to fetch Azure DevOps work item ${reference.id}: ${response.status} ${response.statusText}`, 'AZURE_DEVOPS_FETCH_FAILED');
        }
...
function extractOrganization(url, fallbackOrganization, fallbackProject) {
...
function toAzureWorkItem(payload, reference) {
